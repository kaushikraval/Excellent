...
export const confirmOrder = createAsyncThunk(
  ...
  (data, { dispatch }) => {
    return new Promise((resolve, reject) => {
      axios
        .post('jewellery-diamond-cart-to-order-with-setting', data)
        .then(({ data }) => {
          if (data.IsSuccess) {
...
export const getSearchDiamondFilterList = createAsyncThunk(
  ...
  (data, { getState }) => {
    return new Promise((resolve, reject) => {
      ...
      newFinalObject.StockStatus = data.StockStatus ? data.StockStatus : '';
      axios
        .post('get-search-stock', newFinalObject)
        .then(({ data }) => {
...
export const removeFromSaveSearchTemplate = createAsyncThunk(
  ...
  (data, { dispatch }) => {
    return new Promise((resolve, reject) => {
      ...
      axios
        .post('remove-saved-search-template', obj)
        .then(({ data }) => {
          if (data.IsSuccess) {
            resolve({ data: data.Result });
            dispatch(
...
export const getStockDetailDna = createAsyncThunk(
  ...
  data => {
    return new Promise((resolve, reject) => {
      ...
      axios
        .post('get-stock-detail-dna', newFinalObject)
        .then(({ data }) => {
          resolve({ data: data.Result });
        })
        .catch(errors => {
...
export const getSimillarStockList = createAsyncThunk(
  'get-simillar-stock-list',
  data => {
    return new Promise((resolve, reject) => {
      axios
        .post('get-simillar-stock-detail-dna', data)
        .then(({ data }) => {
          if (data.IsSuccess) {
            resolve({ data: data.Result });
          } else {
...
export const getStockCopytoClipboardString = createAsyncThunk(
  ...
  data => {
    return new Promise((resolve, reject) => {
      axios
        .post('Stock-Copy-to-Clipboard', data)
        .then(({ data }) => {
          if (data.IsSuccess) {
            resolve({ data: data.Result });
          } else {
...
export const dashboardSlice = createSlice({
  ...
  extraReducers: {
    ...
    [getSimillarStockList.pending]: (state, action) => {
      state.similarDiamondLoader = true;
    },
    [getSimillarStockList.fulfilled]: (state, action) => {
      state.similarDiamondList = action.payload.data || [];
      state.similarDiamondLoader = false;
    },
    [getSimillarStockList.rejected]: (state, action) => {
    ...
    [getExportStockData.fulfilled]: (state, action) => {
      ...
      if (action.payload.data?.filePath) {
        ...
        // const url = action.payload.data?.filePath;
        const link = document.createElement('a');
        link.href = outputFilename;
        link.setAttribute('download', outputFilename);
        document.body.appendChild(link);
        link.click();
        // window.open(action.payload.data.filePath);
      }
    },
